      ...
      <div className="auth-card fade-up">
        ...
        <div className="auth-body">
          ...
          <div style={{ marginBottom: 28, textAlign: "center" }}>
            ...
            <div className="font-sans" style={{ marginTop: 6, fontSize: 13, color: "#555" }}>Welcome back — sign in to continue</div>
          </div>

          {/* Error */}
          {error && (
            <div className="font-sans" style={{ display: "flex", alignItems: "flex-start", gap: 9, padding: "12px 14px", borderRadius: 12, background: "rgba(220,38,38,.06)", border: "1px solid rgba(220,38,38,.2)", fontSize: 13, color: "#f87171", marginBottom: 20, lineHeight: 1.5 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <button className="link-btn" style={{ fontSize: 12 }} onClick={() => router.push("/forgot-password")}>Forgot password?</button>
              </div>
              <div style={{ position: "relative" }}>
                <span className="field-icon"><Lock size={15} /></span>
                <input className="field" type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={onKey} placeholder="••••••••" autoComplete="current-password" />
              </div>
            </div>
          </div>

          <button className="btn-submit" style={{ marginTop: 22 }} onClick={handleLogin} disabled={loading}>
            {loading
              ? <><div style={{ width: 17, height: 17, border: "2px solid rgba(255,255,255,.3)", borderTop: "2px solid white", borderRadius: "50%", animation: "spin .7s linear infinite" }} /> Signing in…</>
              : <>Sign In <ArrowRight size={16} /></>
            }
          </button>

          <div className="font-sans" style={{ marginTop: 22, paddingTop: 18, borderTop: "1px solid #1a1a1a", textAlign: "center", fontSize: 14, color: "#555" }}>
            Don't have an account?{" "}
            <button className="link-btn" onClick={() => router.push("/register")}>Create account</button>
          </div>
        </div>
      </div>
