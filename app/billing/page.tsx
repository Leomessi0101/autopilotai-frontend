...
import {
  ...
  TrendingUp, ChevronRight, User, RotateCcw, Sparkles,
} from "lucide-react";

type MeResponse = {
  name: string; email: string; subscription: string;
  used_generations: number; monthly_limit: number | null;
  remaining_generations: number | null; last_reset: string | null;
};

const PLAN_INFO: Record<string, { label: string; desc: string; color: string; features: string[] }> = {
  free:       { label: "Free",       desc: "Limited access to get started.",             color: "#555",    features: ["Basic content generation", "Email writer", "Ad generator", "5 saves per month"] },
  basic:      { label: "Basic",      desc: "Essential tools for individuals.",            color: "#0ea5e9", features: ["All Free features", "100 generations/month", "Growth Pack access", "Priority support"] },
  growth:     { label: "Growth",     desc: "Unlimited for scaling businesses.",           color: "#059669", features: ["Unlimited generations", "AI image creation", "Advanced analytics", "Priority processing"] },
  enterprise: { label: "Enterprise", desc: "Maximum performance, dedicated support.",    color: "#8b5cf6", features: ["Everything in Growth", "Custom integrations", "Dedicated account manager", "SLA guarantee"] },
};
...
export default function BillingPage() {
  ...
  if (!data) return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: "#555" }}>Unable to load billing info. Please try again.</p>
    </div>
  );
  ...
  const usedPct    = data.monthly_limit ? Math.min(100, Math.round(((data.used_generations ?? 0) / data.monthly_limit) * 100)) : 0;
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "white" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Plan features: 1 col on mobile, 2 on medium+ */
        .features-grid { display: flex; flex-direction: column; gap: 8; }
        @media (min-width: 480px) {
          .features-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8; }
        }

        /* CTA row: stack on mobile */
        .cta-row { display: flex; flex-direction: column; gap: 12; }
        @media (min-width: 480px) {
          .cta-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12; align-items: center; }
        }

        .card-pad { padding: 22px 20px; }
        @media (min-width: 560px) { .card-pad { padding: 28px 32px; } }
        ...
        .btn-primary { padding: 14px 24px; border-radius: 12px; border: none; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px; background: linear-gradient(135deg, #059669, #0ea5e9); color: white; transition: filter .2s, box-shadow .2s; width: 100%; justify-content: center; -webkit-tap-highlight-color: transparent; }
        .btn-primary:hover:not(:disabled) { filter: brightness(1.08); box-shadow: 0 10px 32px rgba(5,150,105,.3); }
        .btn-primary:disabled { opacity: .5; cursor: not-allowed; }

        .btn-white { padding: 14px 24px; border-radius: 12px; border: none; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px; background: white; color: #111; transition: background .18s; width: 100%; justify-content: center; -webkit-tap-highlight-color: transparent; }
        .btn-white:hover:not(:disabled) { background: #eee; }
        .btn-white:disabled { opacity: .5; cursor: not-allowed; }

        .btn-ghost { background: transparent; border: 1px solid #2a2a2a; border-radius: 10px; padding: 10px 14px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; color: #888; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; transition: border-color .18s, color .18s, background .18s; -webkit-tap-highlight-color: transparent; }
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 16px 80px" }}>
        ...
        <div className="a1" style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div>
              ...
              <p className="font-sans" style={{ fontSize: 14, color: "#555", marginTop: 8 }}>
        ...
        <div className="a2 billing-grid">
            ...
            <div className="card">
              ...
              <div className="card-pad">
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 18 }}>
                  <div>
                    <div className="section-label" style={{ marginBottom: 8 }}>Current Plan</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      <span className="font-display" style={{ fontSize: "clamp(28px, 5vw, 36px)", color: "white", letterSpacing: "-0.03em", lineHeight: 1 }}>
                        {plan.label}
                      </span>
                      <span className="font-sans" style={{ padding: "3px 10px", borderRadius: 100, fontSize: 11, fontWeight: 800, letterSpacing: ".06em", textTransform: "uppercase", background: `${plan.color}18`, color: plan.color, border: `1px solid ${plan.color}30` }}>
                  ...
                  <div className="font-sans" style={{ fontSize: 13, color: "#555" }}>
                    ...
                    <div style={{ color: "#ccc", fontWeight: 600, wordBreak: "break-all" }}>{data.email}</div>
                  </div>
                </div>

                <div className="features-grid" style={{ marginBottom: 20 }}>
                  {plan.features.map((f) => (
            ...
            <div className="card">
              ...
              <div className="card-pad">
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: `${usageColor}18`, border: `1px solid ${usageColor}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                {data.monthly_limit === null ? (
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                    <span className="font-display" style={{ fontSize: "clamp(32px, 6vw, 42px)", color: "white", letterSpacing: "-0.03em", lineHeight: 1 }}>
                      {data.used_generations ?? 0}
                    </span>
                    <span className="font-sans" style={{ fontSize: 14, color: "#555" }}>generations used</span>
                    <span className="font-sans" style={{ fontSize: 12, padding: "3px 10px", borderRadius: 100, background: "rgba(5,150,105,.12)", color: "#4ade80", border: "1px solid rgba(5,150,105,.25)", fontWeight: 700 }}>
                      Unlimited
                    </span>
                  </div>
                ) : (
                  <>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
                      ...
                      <span className="font-sans" style={{ fontSize: 14, color: "#555" }}>of {data.monthly_limit} used</span>
                    </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span className="font-sans" style={{ fontSize: 12, color: "#555" }}>{data.remaining_generations} remaining</span>
                      <span className="font-sans" style={{ fontSize: 12, color: usedPct > 75 ? usageColor : "#555", fontWeight: usedPct > 75 ? 700 : 400 }}>{usedPct}% used</span>
                    </div>
                    {usedPct > 80 && (
                      <div style={{ marginTop: 14, padding: "12px 14px", borderRadius: 10, background: "rgba(239,68,68,.06)", border: "1px solid rgba(239,68,68,.18)", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {/* ════ RIGHT SIDEBAR ════ */}
          <div className="billing-sidebar" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="card" style={{ padding: "18px 20px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div className="font-sans" style={{ fontSize: 12, color: "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{data.email}</div>
                </div>
              </div>
            ...
            {/* Upgrade nudge (free) */}
            {isFree && (
              <div className="card" style={{ padding: "18px 20px", background: "rgba(5,150,105,.04)", borderColor: "rgba(5,150,105,.18)" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <Sparkles size={14} style={{ color: "#059669", flexShrink: 0, marginTop: 1 }} />
                  <div>
                    <div className="font-sans" style={{ fontSize: 13, fontWeight: 700, color: "#4ade80", marginBottom: 5 }}>Unlock the full suite</div>
            ...
            {!isFree && (
              <div className="card" style={{ padding: "18px 20px" }}>
                ...
                {[
                  { label: "Update payment method", sub: "Change card or billing details" },
                  { label: "Download invoices",      sub: "Get receipts from Stripe portal" },
                  { label: "Cancel subscription",    sub: "Manage via Stripe portal" },
                ].map((item) => (
                  <button key={item.label} className="btn-ghost" onClick={openStripePortal} style={{ width: "100%", justifyContent: "flex-start", padding: "11px 14px", borderRadius: 12, marginBottom: 7, gap: 10 }}>
            ...
            <div className="card" style={{ padding: "18px 20px" }}>
              ...
              <p className="font-sans" style={{ fontSize: 12, color: "#555", lineHeight: 1.65, marginBottom: 10 }}>
                Billing questions? We're here to help.
              </p>
              <a href="mailto:contact@autopilotai.dev" className="font-sans" style={{ fontSize: 13, fontWeight: 600, color: "#0ea5e9", textDecoration: "none" }}>
